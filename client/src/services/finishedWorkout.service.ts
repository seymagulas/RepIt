...
export const getAllFinishedWorkouts = async (): Promise<IFinishedWorkout[]> => {
  try {
    const response = await axios.get(`${API_URL}/finishedWorkouts`, {
      headers: authHeader(),
    });
...
export const getFinishedWorkout = async (
  finishedWorkoutId: string
): Promise<IFinishedWorkout> => {
  try {
...
export const createFinishedWorkout = async (data: IFinishedWorkout) => {
  try {
    const response = await axios.post(`${API_URL}/finishedWorkouts`, data, {
      headers: authHeader(),
    });
    return response.data;
  } catch (error) {
